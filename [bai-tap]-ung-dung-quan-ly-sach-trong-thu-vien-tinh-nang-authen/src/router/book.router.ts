...
import authorAdmin from '../middleware/author.middleware';
...
bookRoutes.use("/post", upload.none(), authMiddleware); // upload.none() chỉ dùng cho middleware đầu tiên, còn lại sau đấy k cần dùng, dùng là xóa hết req.body đi thành {}
bookRoutes.use("/post/create", authorAdmin);
bookRoutes.use("/post/update", authorAdmin);
bookRoutes.use("/post/delete", authorAdmin);
...
bookRoutes.post('/post/create', async (req, res) => {
    try {
        // tạo 1 model publisher mới
        // console.log("req create", req.body.role)
        console.log("req decoded", req["decoded"].role)
        ...
        if (publisherReturn && book && categoryReturn) {
            res.render("success");
        } else {
            res.render("error");
        }
...
bookRoutes.post('/post/update/:id', upload.none(), async (req, res) => {
    try {
